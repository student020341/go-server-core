...
func GetRequestBody(r *http.Request) map[string]interface{} {
	...
	// TODO - figure out how to determine whether ParseForm or a decoder should be used
	...
	// body is drained by ParseForm, but I may need it to decode the request body instead, so...
	// shove it back in - there's probably a better way to do this
	suppository := ioutil.NopCloser(bytes.NewBuffer(buf))
	r.Body = suppository
	r.ParseForm()
	...
	// decode json
	err = json.NewDecoder(bodyCopy).Decode(&obj)
	if err != nil {
		if err != io.EOF {
			fmt.Printf("core: error parsing body: %+v\n", err)
		}
		return nil
	}
